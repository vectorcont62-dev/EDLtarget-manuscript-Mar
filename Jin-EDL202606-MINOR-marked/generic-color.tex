...
The model is benchmarked against TCAD simulations calibrated \hl{only} to the time scale of reported cryogenic retention data and configured to isolate detrapping loss by suppressing thermally activated transitions during retention.
...
However, direct TCAD simulation at \sout{cryogenic temperature about 4.2~K condition}\hl{approximately 4.2~K} did not converge reliably in our setup.
{Therefore, two convergence tendencies were compared in the extreme condition: one obtained by lowering the temperature \hl{from 300~K to} the lowest convergent \sout{cryogenic range} \hl{temperature of 83~K}, and the other obtained by reducing the electron capture cross section \sout{toward zero}\hl{from $10^{-16}$ to $10^{-31}$~cm$^2$}.}
\hl{As $\sigma_n$ decreased, the retention transient approached a limiting curve whose time dependence was consistent with the temperature lowering result.}
{Both approaches showed a consistent retention-time trend in the extreme condition, as reflected in the calibrated TCAD curve in Fig.~\ref{fig3}(a), indicating that the suppression of thermally activated transitions can reproduce the dominant cryogenic detrapping behavior.}
...
{Thus, every TCAD simulation result\sout{s} shown in \hbox{Fig.~\ref{fig3}} should be interpreted as a calibrated cryogenic-emulation benchmark rather than a direct 4.2~K TCAD simulation.}
...
\hl{Thus, the present validation is limited to the calibrated cryogenic emulation TCAD benchmark and does not establish the absolute experimental cryogenic $\Delta V_\text{th}$ magnitude or full device physics at 4.2~K.}
...
\sout{In the model calculation, the trap energy was divided into 100 bins, and the CTN layer was discretized into 30 slices{, which were confirmed to be in the converged regime}.}
\hl{Convergence tests over 10-140 trap energy bins and 5-50 radial CTN slices showed negligible changes beyond 100 bins and 30 slices, which were therefore used in the model calculation.}
...
Additionally, \sout{compared to TCAD simulations calibrated with physical parameters from various prior studies including cryogenic condition, the proposed model {reproduced the calibrated TCAD cryogenic-emulation benchmark} within extremely short timeframe}\hl{the proposed model reproduced the calibrated cryogenic-emulation TCAD benchmark within an extremely short runtime}. 
