...
\begin{figure}[t!]
    ...
    (b) the radial trapped charge distribution used as the model initial condition, 
...
The retention deck includes thermionic emission, Poole--Frenkel emission, and nonlocal tunneling, providing a TCAD reference with thermal and tunneling detrapping mechanisms~\cite{TCADmanual.2022}.
...
{In the refresh calculation, the trapped-charge distribution is sequentially updated after each retention interval and used as the initial condition for the next step, whereas the proposed method directly predicts the transient from the initial distribution without intermediate updates.
...
Therefore, the time-step-free simulation was chosen as the main method of this study since the iterative refresh test test simply multiplies the simulation time by the total number of iterations.
