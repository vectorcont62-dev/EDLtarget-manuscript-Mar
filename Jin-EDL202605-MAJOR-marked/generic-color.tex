...
\begin{figure}[!t]
    ...
    (b) shows \sout{the programmed} \hl{TCAD-extracted post-program} trapped charge distribution, 
...
\begin{figure}[t!]
    ...
    (b) \sout{a detail programmed trapped charge distribution after program operation} \hl{the TCAD-aligned modeled radial trapped-charge profile~\hbox{\cite{C.Lee.JJAP2026,H.Choi.TED2024}} used as the initial condition of the time-step-free model}, 
...
Then, \hl{the} detrapping relation can be express\hl{ed} as \hl{the} following \hl{closed-form solution of the} autonomous ODE \sout{solution} \hl{for each radial/energy discretized element}.
\begin{equation}
    n_t(r, E_t, t) = \cancel{{\sum_r} \sum_{E_t}}n_{t0}(r, E_t) \exp (-(e_\text{ch} + e_\text{WL})t)
...
\begin{figure}
    ...
    \hl{(c) Comparison between time-step-free simulation and the iterative refresh test for assessing the fixed emission rate approximation.}}
    \label{fig3}
\end{figure}
...
According to \cite{Refaldi.IRPS2025}, cryogenic conditions enable the detrapping component to be \sout{isolated from retention loss} \hl{separated from thermally activated retention-loss components}.
However, direct TCAD simulation at cryogenic temperature about 4.2~K condition did not converge reliably in our setup.
\hl{Therefore, two convergence tendencies were compared in the extreme condition: one obtained by lowering the temperature toward the lowest convergent cryogenic range, and the other obtained by reducing the electron capture cross section toward zero.}
\hl{Both approaches showed the same retention-time tendency in the extreme condition, indicating that the suppression of thermally activated transitions can reproduce the dominant cryogenic detrapping behavior within the convergent TCAD framework.}
...
\hl{Thus, every TCAD simulation results shown in \hbox{Fig.~\ref{fig3}} should be interpreted as a calibrated cryogenic-emulation benchmark rather than a direct 4.2~K TCAD simulation.}

\hl{The TCAD benchmark was built using Synopsys Sentaurus TCAD.}
\hl{The retention deck includes thermionic emission, Poole--Frenkel emission, and nonlocal tunneling, providing a TCAD reference with thermal and tunneling detrapping mechanisms~\hbox{\cite{TCADmanual.2022}}.}
\hl{The present implementation focuses on isolated radial detrapping under cryogenic-emulation conditions, without separately resolving explicit lateral charge migration.}
...
\sout{The model validation is performed using Synopsys TCAD simulations calibrated against previous retention modeling studies~\hbox{\cite{Han.TED2026, Jin.TED2025}}.}
...
% \hl{This is because $t_\text{TOX}$ directly controls the tunneling probability from CTN traps toward the channel side, making it the most sensitive geometrical parameter for the detrapping transient.}
In Fig.~\ref{fig3}(b), $t_\text{TOX}$ from 3~nm to 5~nm for both TCAD simulation and the model were performed, showing good \sout{match}\hl{agreement} with each other.
% \hl{The model accurately reproduces the oxide-thickness-dependent retention transient observed in the TCAD benchmark.}
\hl{To verify the time-step-free formulation, an iterative refresh test was performed as shown in Fig.~\hbox{\ref{fig3}(c)}.}
\hl{In the proposed closed-form calculation, $e_\mathrm{ch}$ and $e_\mathrm{WL}$ are evaluated from the initially programmed state and kept fixed.}
\hl{In the refresh calculation, the trapped-charge distribution is sequentially updated after each retention interval.}
\hl{Both approaches show nearly identical $\Delta V_\text{th}$ behavior over the entire retention range, with only about 3\% difference from time exceeding $10^9$~s.}
\hl{Therefore, the time-step-free simulation was chosen as the main method of this study since the iterative refresh test simply multiplies the simulation time by the total number of iterations.}
...
Since the model's computation time is independent \sout{from}\hl{of} sequential time \sout{step,}\hl{stepping, the proposed method} can be adopted to both short- and long-term retention \sout{operation based on given physical time}\hl{prediction for arbitrary target retention times}.
...
Additionally, compared to TCAD simulations calibrated with physical parameters from various prior studies including cryogenic condition, the proposed model \sout{demonstrates good match with TCAD simulation}\hl{reproduced the calibrated TCAD cryogenic-emulation benchmark} within extremely short timeframe. 
