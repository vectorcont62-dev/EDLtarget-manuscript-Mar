...
Thus, the present benchmark supports the retention-time-scale consistency and TCAD-model agreement for this selected programmed state.
This scope limitation applies to the present validation benchmark, not to the time-step-free formulation itself, which can be applied to other specified initial trapped-charge distributions and arbitrary queried retention times.
However, this work does not claim full experimental validation of the absolute cryogenic $\Delta V_\mathrm{T}$ magnitude or complete temperature dependence.
