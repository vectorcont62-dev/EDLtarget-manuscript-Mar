...
The model is benchmarked against TCAD simulations calibrated {only} to the time scale of reported cryogenic retention data and configured to isolate detrapping loss by suppressing thermally activated transitions during retention.
...
However, direct TCAD simulation at {approximately 4.2~K} did not converge reliably in our setup.
{Therefore, two convergence tendencies were compared in the extreme condition: one obtained by lowering the temperature {from 300~K to} the lowest convergent {temperature of 83~K}, and the other obtained by reducing the electron capture cross section {from $10^{-16}$ to $10^{-31}$~cm$^2$}.}
{As $\sigma_n$ decreased, the retention transient approached a limiting curve whose time dependence was consistent with the temperature lowering result.}
...
{Thus, every TCAD simulation result shown in \hbox{Fig.~\ref{fig3}} should be interpreted as a calibrated cryogenic-emulation benchmark rather than a direct 4.2~K TCAD simulation.}
...
{Thus, the present validation is limited to the calibrated cryogenic emulation TCAD benchmark and does not establish the absolute experimental cryogenic $\Delta V_\text{th}$ magnitude or full device physics at 4.2~K.}
...
{Convergence tests over 10-140 trap energy bins and 5-50 radial CTN slices showed negligible changes beyond 100 bins and 30 slices, which were therefore used in the model calculation.}
...
Additionally, {the proposed model reproduced the calibrated cryogenic-emulation TCAD benchmark within an extremely short runtime}. 
