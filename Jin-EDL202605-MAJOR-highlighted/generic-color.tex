...
\hl{Therefore, before applying the cryogenic-emulation condition, two convergence tendencies were compared in the extreme condition: one obtained by lowering the temperature toward the lowest convergent cryogenic range, and the other obtained by reducing the electron capture cross section toward zero.}
...
\hl{Based on this convergence tendency, the calibration in \hbox{Fig.~\ref{fig3}(a)} was performed using the original time-scale matching strategy, without introducing an additional calibration figure.}
\hl{Thus, \hbox{Fig.~\ref{fig3}(a)} should be interpreted as a calibrated cryogenic-emulation benchmark rather than a direct 4.2~K TCAD simulation.}

\hl{The TCAD benchmark was built using Synopsys Sentaurus TCAD following the Device manual~\hbox{\cite{TCADmanual.2022}}.}
\hl{The retention deck includes thermionic emission, Poole--Frenkel emission, and nonlocal tunneling, providing a TCAD reference with thermal and tunneling detrapping mechanisms.}
...
\hl{Both approaches show nearly identical $\Delta V_\text{th}$ behavior over the entire retention range, with only about 2\% difference at $10^9$~s.}
