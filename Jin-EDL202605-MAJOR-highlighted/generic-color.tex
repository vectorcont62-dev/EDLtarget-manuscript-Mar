...
\begin{figure}[t!]
    ...
    (b) \sout{a detail programmed trapped charge distribution after program operation} \hl{the radial trapped charge distribution used as the model initial condition}, 
...
This autonomous detrapping equation (\ref{autonomousSoluation}) is the key relation that enables a simulation to directly compute the result at a specific time once initially programmed trapped charge distribution is specified.
...
\begin{figure}
    ...
    \hl{(c) Comparison between time-step-free simulation to iteratively refreshed simulation showing almost no difference.}}
    \label{fig3}
\end{figure}
...
According to \cite{Refaldi.IRPS2025}, cryogenic conditions enable the detrapping component to be \sout{isolated from retention loss} \uline{separated from thermally activated retention-loss components}.
...
In Fig.~\ref{fig3}(b), $t_\text{TOX}$ from 3~nm to 5~nm for both TCAD simulation and the model were performed, showing good match with each other.
...
\hl{In the refresh calculation, the trapped-charge distribution is sequentially updated after each retention interval and used as the initial condition for the next step, whereas the proposed method directly predicts the transient from the initial distribution without intermediate updates.}
...
\hl{Therefore, the time-step-free simulation was chosen as the main method of this study since the iterative refresh test test simply multiplies the simulation time by the total number of iterations.}
...
Since the model's computation time is independent from sequential time step, can be adopted to both short- and long-term retention operation based on given physical time.
...
Additionally, compared to TCAD simulations calibrated with physical parameters from various prior studies including cryogenic condition, the proposed model demonstrates good match with TCAD simulation within extremely short timeframe. 
